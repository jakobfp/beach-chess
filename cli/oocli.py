...
class Rally:
    ...
    def print_rally(self):
        ...
        print(f"defence score: {self.defense_score}, attack score: {self.attack_score}")
        ...
        print_commentary(comment, self.attacker.get_attack(), self.defender.get_block(), self.defender.get_block(), self.attacker.get_name(), self.defender.get_name())
    ...
    def calc_outcome(self):
        ...
        self.attack_score = np.random.randint(0, 101)
        self.print_rally()
...
class Game:
    ...
    def coin_toss(self):
        ...
        print(f"{self.players[coin].name} receives serve!")
    ...
    def game_finished(self):
        ...
        if points[0] < 15 and points[1] < 15:
            return False
...
def play_game(mode):
    ...
    assert player1_name != player2_name, "Enter two different names, please!"
    game = Game(Player(player1_name), Player(player2_name), lookup)
    game.coin_toss()
    while not game.game_finished():
        game.play()

    print(f"{Font.PURPLE}{game.score}{Font.END}")


if __name__ == '__main__':
    try:
        play_game(MULTI_PLAYER_MODE)
